...
module.exports = {
  getCodeError(code){
    // src/bridgedesigner/DesignConditions.java
    ...
    const nPanels = 10 * code[1] + code[2];
    if (!(1<=nPanels<=20)) {
      return 2;
    }
    const over = 10 * code[3] + code[4];
    if (!(0<=over<=40)) {
      return 4;
    }
    if (!(0<=code[9]<=1)) {
    ...
    const pierPanelIndex = code[8] - 1;
    const pier = pierPanelIndex >= 0;
    const hiPier = code[9] > 0;
    ...
    if ((pier) && (!hiPier))
      {
        const xp = pierPanelIndex * wpbd.panelSizeWorld;
        const yp = deckElev - under;
        const xL = 0.0;
        const yL = deckElev;
        const xR = nPanels * 4.0;
        const yR = yL;
        if (xp < xL + (yL - yp) * 0.5) {
          return 98;
        }
  ...
  codeToCondition(wpbd, codeLong){
    ...
    const pier = f.pierPanelIndex >= 0;
    // digit 8 => (0 = simple, 1 = arch, 2 = cable left, 3 = cable both)
    const arch = f.code[7] == 1;
    const leftCable = (f.code[7] == 2) || (f.code[7] == 3);
    const rightCable = f.code[7] == 3;
    ...
    const loadCaseIndex = f.code[0] - 1;
